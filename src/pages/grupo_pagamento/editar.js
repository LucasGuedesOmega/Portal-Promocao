...
export function EditarGrupoPagamento(){

    const { id_grupo_pagamento } = useParams();
    let token = localStorage.getItem('tokenApi');
    const navigate = useNavigate();

    return (<Editar id_grupo_pagamento={id_grupo_pagamento} token={token} navigate={navigate}/>);
};

export function CadastrarGrupoPagamento(){

    let token = localStorage.getItem('tokenApi');
    const navigate = useNavigate();

    return (<Editar id_grupo_pagamento={null} token={token} navigate={navigate}/>);
}
...
class Editar extends React.Component{
    ...
    async componentDidMount(){
        if (this.state.id_grupo_pagamento){ 
            await this.get_grupo_pagamento()
        }
    }
    ...
    submitForm(){
        ...
        try{
            api.post('/api/v1/grupo-pagamento', data_grupo_pagamento, { headers: { Authorization: this.props.token}})
            .then((results) => {
                console.log(results)
                if (results.data['Sucesso']){
                    if (this.state.id_grupo_pagamento){
                        message = 'Grupo de Pagamento editado com sucesso!'
                    } else{
                        message = 'Grupo de pagamento cadastrado com sucesso!'
                    }
            ...
            <div className='cadastro'>
                <div  className="cadastro__formulario" >
                    <div className="cadastro__formulario__header">
                        <div className="row">
                            <div className="col-md-10"><h3 className="cadastro__formulario__header__titulo">Grupo de Pagamento</h3></div>
                        </div>
                    </div>
                    <hr />
